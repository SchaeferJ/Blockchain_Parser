...
def make_sends_list(transaction: dict) -> list:
    ...
    for i, input_tx in enumerate(tx_in):
        ...
        inputs.append([input_add_tmp, input_value_tmp, txid, 'SENDS'])
    return inputs
...
for block_height in trange(START_BLOCK, END_BLOCK):
    ...
    before_file_w.writerow([block_hash, next_block_hash, 'PRECEDES'])
    ...
    for i, tx_hash in enumerate(block_contents['tx']):
        ...
        try:
            receives = list(
                map(lambda x: [tx_hash, x['value'], x['n'], x['scriptPubKey']['addresses'][0], 'RECEIVES'],
                    transaction_output))
        ...
        belongs_file_w.writerow([tx_hash, block_hash, 'BELONGS_TO'])
